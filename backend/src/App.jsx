    ...
    <div style={{textAlign:"center",padding:"16px 0"}}>
      ...
      <div style={{background:T.surf,border:`2px solid ${T.gold}`,borderRadius:"12px",padding:"28px",maxWidth:"430px",margin:"0 auto 28px",textAlign:"left"}}>
        <div style={{borderBottom:`1px solid ${T.brd}`,paddingBottom:"14px",marginBottom:"16px",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
          {/* MongoDB devuelve _id, usamos || para compatibilidad */}
          <div style={{fontFamily:"'Cormorant Garamond',serif",fontSize:"19px",fontWeight:600}}>Reserva #{confirmed?._id || confirmed?.id}</div>
  ...
  return (
    <div style={{maxWidth:"680px",margin:"0 auto",padding:"48px 24px"}}>
      ...
      <div style={{background:"#FFF9EF",border:`1px solid ${T.goldL}`,borderRadius:"6px",padding:"11px 16px",marginBottom:"22px",fontSize:"12px",color:T.taupe}}>
        💡 Ingresa el correo con el que agendaste tu cita.
      </div>
          ...
          {apts.length===0
            ...
            : <div style={{display:"flex",flexDirection:"column",gap:"14px"}}>
                {apts.map(a => {
                  ...
                  return (
                    <div key={a._id||a.id} style={{background:T.surf,border:`1px solid ${T.brd}`,borderRadius:"10px",padding:"22px",opacity:can?.6:1}}>
                      ...
                      {!can && (
                        <button onClick={()=>cancel(a._id||a.id)}
...
function Admin({ tick, setTick }) {
  ...
  const stats = useMemo(() => {
    ...
    const tod  = all.filter(a => a.date===todayS() && a.status==="confirmed");
    const rev  = conf.reduce((s,a) => s+(gSvc(a.serviceId)?.price||0), 0);
    return { total:all.length, confirmed:conf.length, today:tod.length, revenue:rev };
  }, [all]);
  ...
  return (
    <div style={{maxWidth:"1100px",margin:"0 auto",padding:"48px 24px"}}>
      ...
      <div style={{background:T.surf,border:`1px solid ${T.brd}`,borderRadius:"10px",overflow:"hidden"}}>
        <div style={{padding:"14px 22px",borderBottom:`1px solid ${T.brd}`}}>
              ...
              <table style={{width:"100%",borderCollapse:"collapse",fontSize:"12px"}}>
                ...
                <tbody>
                  {rows.map((a,i) => {
                    const svc=gSvc(a.serviceId), stf=gStf(a.staffId), can=a.status==="cancelled";
                    return (
                      <tr key={a._id||a.id} style={{borderTop:`1px solid ${T.brd}`,background:i%2===0?"transparent":"#FAFAF8",opacity:can?.6:1}}>
                        <td style={{padding:"11px 14px",color:T.muted,fontFamily:"monospace",fontSize:"10px"}}>{(a._id||a.id)?.toString().slice(-6)}</td>
                        ...
                        <td style={{padding:"11px 14px"}}>
                          {!can && (
                            <button onClick={()=>cancelRow(a._id||a.id)}
...
export default function App() {
  ...
  // ← ESTE es el doConfirm correcto: usa fetch para guardar en MongoDB Atlas
  const doConfirm = async () => {
    try {
      ...
      alert("Error al guardar la cita. Verifica que el servidor esté corriendo en localhost:3000");
    }
  };

  return (
    <div style={{minHeight:"100vh",background:T.bg,fontFamily:"'DM Sans',sans-serif",color:T.dark}}>
      <style>{`
        *{box-sizing:border-box;margin:0;padding:0}
        input:focus,select:focus{outline:none;border-color:#B8965A!important;box-shadow:0 0 0 3px rgba(184,150,90,.14)!important}
      ...
      {view==="book"   && <Booking step={step} setStep={setStep} bk={bk} setBk={setBk} doConfirm={doConfirm} confirmed={confirmed} nav={nav} />}
      {view==="myapts" && <MyApts tick={tick} setTick={setTick} />}
      {view==="admin"  && <Admin  tick={tick} setTick={setTick} />}
    </div>
  );
}